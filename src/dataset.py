...
class ImagesDataset(Dataset):
    ...
    def __init__(self, image_dir, width: int = 100, height: int = 100, dtype: Optional[type] = None, transform=None):
        ...
        if width < 100 or height < 100:
            raise ValueError('width and height must be greater than or equal to 100')
        self.width = width
        self.height = height
        self.dtype = dtype
    ...
    def __getitem__(self, index):
        with Image.open(self.image_filepaths[index]) as im:
            ...
            if self.transform:
                ...
                im = np.array(im, dtype=self.dtype)
        ...
        return im, classid, classname, self.image_filepaths[index]
